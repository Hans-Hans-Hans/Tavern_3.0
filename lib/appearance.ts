...
export type AppearancePreferences = { version: 1; theme: 'system' | 'light' | 'dark'; font: 'system' | 'readable' | 'monospace'; chatScale: number; memberList: boolean; reducedMotion: 'system' | 'always'; saturation: number };
...
export function saveAppearance(change: Partial<AppearancePreferences>, c = getMatrixClient()) { if (!c) throw new Error('Sign in to save appearance preferences.'); const task = writes.catch(() => {}).then(async () => { const saved = await c.getAccountDataFromServer(appearanceKey as any), next = normalizeAppearance({ ...readAppearance(c), ...saved, ...change }); await c.setAccountData(appearanceKey as any, next as any); applyAppearance(next); return next; }); writes = task; return task; }
