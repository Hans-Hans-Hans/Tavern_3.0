...
export async function saveServerLayout(serverId: string, layout: ServerLayout) { const { c, me, room } = context(serverId); if (!room!.isSpaceRoom() || !room!.currentState.maySendStateEvent(communityEvents.layout, me)) throw new Error('You do not have permission to organize this server.'); await c.sendStateEvent(serverId, communityEvents.layout as any, normalizeServerLayout(layout, serverChannelIds(serverId)), ''); }
export function readServerBranding(serverId: string) { const r = getMatrixClient()?.getRoom(serverId), p = state(r, 'io.tavern.server.branding'); return { name: r?.name || '', description: clean(state(r, 'm.room.topic').topic, 1000), icon: cleanMxc(state(r, 'm.room.avatar').url), banner: cleanMxc(p.banner), accent: color(p.accent), welcome: clean(p.welcome, 2000) }; }
export function canEditServerBranding(serverId: string) { const c = getMatrixClient(), r = c?.getRoom(serverId), me = c?.getUserId(); return !!(r?.isSpaceRoom() && me && ['m.room.name', 'm.room.topic', 'm.room.avatar', 'io.tavern.server.branding'].every(type => r.currentState.maySendStateEvent(type, me))); }
export async function saveServerBranding(serverId: string, value: ReturnType<typeof readServerBranding>) { const { c } = context(serverId); if (!canEditServerBranding(serverId)) throw new Error('You do not have permission to customize this server.'); const name = clean(value.name, 60).trim(); if (!name) throw new Error('Enter a server name.'); const writes: [string, object][] = [['m.room.name', { name }], ['m.room.topic', { topic: clean(value.description, 1000) }], ['m.room.avatar', { url: cleanMxc(value.icon) }], ['io.tavern.server.branding', { banner: cleanMxc(value.banner), accent: color(value.accent), welcome: clean(value.welcome, 2000) }]]; let saved = 0; try { for (const [type, content] of writes) { await c.sendStateEvent(serverId, type as any, content, ''); saved++; } } catch (error) { throw new Error(`${saved ? 'Some server details were saved. ' : ''}${(error as Error).message} Reopen and retry.`); } }
...
export function readOwnProfile(serverId?: string): Profile {
  const c = getMatrixClient(), me = c?.getUserId() || '', user = c?.getUser(me), p = serverId ? state(c?.getRoom(serverId), 'm.room.member', me) : null;
  return normalizeProfile(p ? { ...p[communityEvents.profile], name: p.displayname, avatar: p.avatar_url } : { name: user?.displayName || me, avatar: user?.avatarUrl, ...c?.getAccountData(communityEvents.profile as any)?.getContent() });
}
