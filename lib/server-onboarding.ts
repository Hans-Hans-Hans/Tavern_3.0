...
export type ServerOnboarding = { version: 1; enabled: boolean; startChannel: string; welcomeChannel: string; rulesChannel: string; recommended: string[]; interests: { id: string; label: string; channels: string[] }[] };
...
export async function saveServerOnboarding(serverId: string, value: ServerOnboarding, previous: ServerOnboarding) {
  ...
  if ([next.startChannel, next.welcomeChannel, next.rulesChannel, ...next.recommended, ...next.interests.flatMap(interest => interest.channels)].some(id => id && !allowed.has(id))) throw new Error('Choose channels that still belong to this server.');
