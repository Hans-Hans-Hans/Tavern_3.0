...
export function updateNotificationPreferences(mutate: (old: NotificationPreferences) => NotificationPreferences, client = getMatrixClient(), active: () => boolean = () => true) { if (!client) throw new Error('Sign in to change notification preferences.'); const current = () => { if (!active()) throw new Error('Your account or room membership changed.'); }; const task = queue.catch(() => {}).then(async () => { current(); const fresh = await client.getAccountDataFromServer(notificationPreferencesKey as any); current(); const next = normalizeNotificationPreferences(mutate(normalizeNotificationPreferences(fresh))); current(); await client.setAccountData(notificationPreferencesKey as any, next as any); current(); return next; }); queue = task; return task; }
