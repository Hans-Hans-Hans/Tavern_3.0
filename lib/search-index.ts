...
export type SearchDocument = { id: string; roomId: string; roomName: string; authorId: string; authorName: string; body: string; timestamp: number; editedAt: number; has: ('image' | 'file' | 'link')[]; mentions: string[]; mentionRoom: boolean };
...
export function matchesSearch(doc: SearchDocument, query: SearchQuery, me: string) {
  const words = new Set(searchWords(doc.body)), body = doc.body.normalize('NFKC').toLocaleLowerCase();
...
export async function sealSearchDocument(document: SearchDocument, keys: Keys): Promise<StoredDocument> {
  ...
  const tags = await Promise.all(searchWords(document.body).map(word => searchTag(keys.search, word)));
...
export async function searchMessages(input: string, options: { limit?: number; cursor?: string; roomIds?: string[]; currentUser?: string } = {}) {
  const s = sessionRequired(), query = parseSearchQuery(input), tags = await Promise.all(query.terms.map(term => searchTag(s.keys.search, term))), limit = Math.max(1, Math.min(100, options.limit || 50)), allowed = options.roomIds ? new Set(options.roomIds) : null, ignored = new Set(s.client.getIgnoredUsers()), hits: SearchDocument[] = [];
  let cursor = decodeCursor(options.cursor), scanned = 0, more = false;
  while (scanned < 2000 && hits.length < limit && isActive(s)) {
    const rows = await candidates(s, tags[0], cursor, query, Math.min(200, 2000 - scanned)); if (!rows.length) { more = false; break; } more = true;
    for (const row of rows) { cursor = [row.timestamp, row.id]; scanned++; if (joined(s, row.roomId) && (!allowed || allowed.has(row.roomId)) && tags.every(tag => row.tags.includes(tag))) { try { const doc = await openSearchDocument(row, s.keys); if (!ignored.has(doc.authorId) && matchesSearch(doc, query, options.currentUser || s.client.getUserId() || '')) hits.push(doc); } catch { await removeDocument(s, row.id); } } if (hits.length >= limit || scanned >= 2000) break; }
    if (rows.length < 200 && hits.length < limit) { more = false; break; }
  }
  if (!isActive(s)) throw new Error('Your search session changed. Retry after signing in.');
  return { hits, nextCursor: more && cursor ? cursorOf({ timestamp: cursor[0], id: cursor[1] }) : null, indexedCount: (await searchIndexStatus()).indexedCount, scanned };
}
...
export async function clearSearchIndex() { const s = sessionRequired(); await s.queue; const tx = s.database.transaction('documents', 'readwrite'); tx.objectStore('documents').clear(); await transactionDone(tx); }
