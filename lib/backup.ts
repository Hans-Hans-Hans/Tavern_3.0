...
type BackupInfo={version:string;algorithm:string;auth_data:{public_key:string}};
 ...
 */
export async function createOrResumeBackup(c:MatrixClient){
  const crypto=c.getCrypto()!;
  const {BackupDecryptionKey,initAsync}=await import('@matrix-org/matrix-sdk-crypto-wasm');await initAsync();
