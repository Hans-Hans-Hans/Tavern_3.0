...
export async function exportMessageHistory(client: MatrixClient, roomIds: string[], ownOnly: boolean, write: (line: string) => Promise<void>, onProgress: (value: ExportProgress) => void, signal: AbortSignal) {
  const started = Date.now(), userId = client.getUserId(), device = client.getDeviceId(), progress: ExportProgress = { room: '', roomsDone: 0, roomsTotal: roomIds.length, examined: 0, exported: 0, undecryptable: 0 };
  const check = () => { signal.throwIfAborted(); if (client !== getMatrixClient() || client.getUserId() !== userId || client.getDeviceId() !== device) throw new Error('Your account changed. The export was stopped.'); };
  ...
  for (const roomId of [...new Set(roomIds)]) {
    ...
    const checkRoom = () => { check(); if (client.getRoom(roomId) !== room || room.getMyMembership() !== 'join') throw new Error('Your selected room membership changed. The export was stopped.'); };
    ...
    while (true) {
      checkRoom(); const page = await client.createMessagesRequest(roomId, token, 100, Direction.Backward); checkRoom();
      ...
      for (const raw of page.chunk) {
        ...
        if (event.isEncrypted()) { await client.decryptEventIfNeeded(event).catch(() => {}); checkRoom(); }
      ...
      checkRoom(); onProgress({ ...progress }); checkRoom();
      // A filtered page can be empty while older accessible events remain.
      if (page.end === undefined) break;
      if (typeof page.end !== 'string' || !page.end || page.end === token || seen.has(page.end)) throw new Error('History pagination stopped advancing. Retry the export after checking your homeserver.');
