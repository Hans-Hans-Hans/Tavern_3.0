...
import { matrixApi as api, connectMatrix, restoreMatrixSession, disconnectMatrix, matrixStatus, onMatrixUpdate, uploadMatrixFile, discardMatrixFile, downloadMatrixFile, exportMatrixMessages, exportEncryptionKeys, importEncryptionKeys, getMatrixClient, resolveMatrixMessage } from '@/lib/matrix';
type Member = {
    id: string;
    name: string;
    role: string;
...
function Workspace() {
    ...
    useEffect(() => { let active = true; readInstanceConfig().then(config => { if (active && config.homeserverUrl)
        { setServer(current => current || config.homeserverUrl); setLockedServer(!!config.lockHomeserver); setRolePolicyEnabled(config.serverRolePolicy===true); } }); return () => { active = false; }; }, []);
    ...
    useEffect(()=>{const due=(event:Event)=>{const item=(event as CustomEvent).detail;toast('A message reminder is due',{action:{label:'Open',onClick:()=>{select(item.roomId);if(item.eventId)void resolveMatrixMessage(item.roomId,item.eventId).then(setThread).catch(e=>toast.error(e.message));}}});};window.addEventListener('tavern:reminder',due);return()=>window.removeEventListener('tavern:reminder',due);},[]);
    ...
    const visibleChannels = data?.conversations.filter(c => c.kind !== 'dm' && (!currentServer || currentServer.roomIds.includes(c.id))).map(c=>({...c,unread:Math.max(c.unread,navPreferences.unread.includes(c.id)?1:0)}));
    ...
    useEffect(() => { let timer: ReturnType<typeof setTimeout>|undefined; const off = onMatrixUpdate(() => { if(timer)return;timer = setTimeout(() => { timer=undefined;setConnection(matrixStatus().state); api('bootstrap').then(b => { setData(b); setPrefs({ ...defaultPrefs, ...b.preferences }); }).catch(() => { }); loadMessages(true); }, 150); }); return () => { off(); clearTimeout(timer); }; }, [loadMessages]);
    ...
    useEffect(() => { if (!thread)
        ...
        setReplies(r.messages); }).catch(e => toast.error(e.message)); setReplies([]); run(); const off = onMatrixUpdate(() => { if (!document.hidden) void run(); }); return () => { live = false; off(); }; }, [thread?.id]);
    ...
    function roomActions(c:Conv):ContextAction[]{const room=getMatrixClient()?.getRoom(c.id),uid=getMatrixClient()?.getUserId();const navPrefs=navigationPreferences();return [
      {label:'Mark read',run:async()=>{const last=room?.getLiveTimeline().getEvents().at(-1);await api('read',{conversation:c.id,id:last?.getId()});await setNavigationFlag(c.id,'unread',false);}},
    ...
    function serverActions(server:{id:string;name:string}):ContextAction[]{const room=getMatrixClient()?.getRoom(server.id),prefs=navigationPreferences();return [{label:'Open server',run:()=>chooseServer(server.id)},{label:prefs.favorites.includes(server.id)?'Remove favorite':'Favorite server',run:()=>setNavigationFlag(server.id,'favorites',!prefs.favorites.includes(server.id))},{label:'Mark server read',run:async()=>{for(const e of room?.currentState.getStateEvents('m.space.child')||[]){const id=e.getStateKey();if(!id)continue;const c=getMatrixClient()?.getRoom(id);if(c?.getMyMembership()==='join'){await api('read',{conversation:id,id:c.getLiveTimeline().getEvents().at(-1)?.getId()});await setNavigationFlag(id,'unread',false);}}}},{label:'Server settings',visible:canEditCommunity(server.id,'layout'),run:()=>{chooseServer(server.id);setModal('serverSettings');}},{label:'Invite people',visible:!!room?.canInvite(getMatrixClient()?.getUserId()||''),run:()=>{chooseServer(server.id);setModal('inviteServer');}},{label:'Copy server link',run:()=>copyText(location.origin+'/#server='+encodeURIComponent(server.id))},{label:'Copy server ID',run:()=>copyText(server.id)},{label:'Report server',visible:isManagedAccount(),run:()=>setReportTarget({kind:'server',roomId:server.id})},{label:'Leave server',danger:true,run:()=>setConfirmAction({title:'Leave '+server.name+'?',description:'This removes your server membership. Leave individual channels separately if you also want to leave those conversations.',run:async()=>{await getMatrixClient()!.leave(server.id);await loadBootstrap();chooseServer('all');}})}];}
    function prefsForMute(id:string){return prefs.muted.includes(id)?prefs.muted.filter(v=>v!==id):[...prefs.muted,id];}
    function userActions(id:string):ContextAction[]{return [
      {label:'View profile',run:()=>setProfileUser(id)},
      ...
      {label:'Report user',visible:isManagedAccount()&&id!==me?.id,run:()=>setReportTarget({kind:'user',targetId:id,roomId:channel})},
    ];}
    function messageCard(m: Msg, inThread = false) {
        if(isUserBlocked(m.author_id))return null;
        const permissions=messagePermissions(m.conversation_id,m.author_id);
        const gallery = (inThread ? [thread,...replies].filter(Boolean) as Msg[] : messages).flatMap(item=>item.attachments);
        const replyAllowed=permissions.send&&!threadReplyRestriction(m.conversation_id,m.parent_id||m.id,m.lastActivity);
        const actions:ContextAction[]=[
          {label:'Reply in thread',visible:replyAllowed,run:()=>{if(m.parent_id)void resolveMatrixMessage(m.conversation_id,m.parent_id).then(setThread).catch(e=>toast.error(e.message));else setThread(m);}},
          ...
          {label:'Quote message',visible:permissions.send,run:()=>{select(m.conversation_id);setTimeout(()=>window.dispatchEvent(new CustomEvent('tavern:quote',{detail:{roomId:m.conversation_id,text:'> '+m.author_name+' wrote:\n'+m.body.split('\n').map(line=>'> '+line).join('\n')+'\n\n'}})),50);}},{label:'Copy text',run:()=>copyText(m.body)},
          {label:'Remind me',run:()=>setReminder({roomId:m.conversation_id,eventId:m.id})},{label:'Copy message link',run:()=>copyText(messageLink(m))},
          {label:'Copy message ID',run:()=>copyText(m.id)},
  ...
  {!inThread && m.replies > 0 && <button className='thread-link' onClick={() => setThread(m)}><MessageCircle size={15}/>{m.replies} {m.replies === 1 ? 'reply' : 'replies'}<span>View thread</span><ChevronRight size={13}/></button>}
   ...
   {permissions.react&&<DropdownMenu><DropdownMenuTrigger asChild><button className='icon-button' aria-label='Add reaction'><Smile size={17}/></button></DropdownMenuTrigger><DropdownMenuContent><EmojiPicker onSelect={emoji=>void act('react',{id:m.id,emoji}).catch(()=>{})}/><ServerEmojiPicker serverId={currentServer?.id} onPick={e=>void act('react',{id:m.id,emoji:e.uri}).catch(()=>{})}/></DropdownMenuContent></DropdownMenu>}
   {!inThread && !m.parent_id && replyAllowed && <IconButton label='Reply in thread' onClick={() => setThread(m)}><MessageSquare size={17}/></IconButton>}
 ...
 <main className='main-workspace'>
  ...
  <div className='conversation-layout'><section className='conversation-main'>
   ...
   {view==='channel'&&tab==='messages'&&forumMode?<ForumChannel roomId={channel} messages={messages} onOpen={setThread} onSent={()=>loadMessages(true)}/>:view==='channel'&&tab==='work'?<CollaborationBoard key={channel} roomId={channel}/>:<div className='message-scroll' ref={scroll} onScroll={() => { if (scroll.current)
    ...
    {loading && messages.length === 0 ? <div className='loading-messages'><Loader2 size={23} className='spin'/><span>Opening your conversations…</span></div> : <>
     ...
     <MessageList key={channel+':'+tab+':'+view} items={(tab==='pins'&&view==='channel'?messages.filter(m=>m.pinned):messages).filter(m=>!isUserBlocked(m.author_id))} scroll={scroll} render={(m,i)=><div>{view==='channel'&&(i===0||new Date(m.created_at).toDateString()!==new Date(messages[i-1]?.created_at).toDateString())&&<div className='date-divider'><span>{new Date(m.created_at).toLocaleDateString([],{weekday:'long',month:'long',day:'numeric'})}</span></div>}{messageCard(m)}</div>}/>
...
function Composer({ conversation, parent, name, members, serverId, onSent, onEditLatest, disabled = false, shareTyping = false }: {
    ...
    async function send() { if(restriction){toast.error(restriction);return;} if (disabled || sending || uploading || (!draft.trim() && !files.length))
        return; setSending(true);setSendFailed(false); nonce.current ??= crypto.randomUUID(); try {
        await api('send', { conversation, parent, serverId, body: draft, attachments: files.map(f => f.id), nonce: nonce.current });
        setDraft('');
        ...
        setSendFailed(true);toast.error((e as Error).message);
    }
    finally {
        setSending(false);
    } }
