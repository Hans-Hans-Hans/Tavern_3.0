...
export function Workspace() {
 ...
 <Sidebar collapsible='offcanvas' className='channel-sidebar'>
  ...
  </SidebarContent><SidebarFooter className='sidebar-footer'><VoiceSidebarDock onSelect={select} onSettings={id=>{select(id);setDetail(true);}}/><div className='connection'><span className={connection === 'Connected' ? 'live' : ''}/>{connection}<span className='version'>v0.4.0</span></div><button className='profile-button' onClick={() => openSettings('profile')}><Avatar name={me?.name || 'You'} size='small'/><span><strong>{me?.name || label("Your Tavern")}</strong><small>{data?.preview ? 'Not connected' : 'Matrix account'}</small></span><Settings size={17}/></button></SidebarFooter>
 </Sidebar>
 ...
 <Sheet open={detail&&!privateRoom&&!thread} onOpenChange={setDetail}><SheetContent className='detail-sheet'><SheetHeader><SheetTitle>{voiceMode?<Headphones/>:active?.kind==='private'?<LockKeyhole/>:<Hash/>}{name}</SheetTitle><SheetDescription>Channel information and management</SheetDescription></SheetHeader>
  <div className='detail-content'><ManagementSections key={channel+':'+managementScope.current.generation} label='Channel settings sections' sections={[
   ...
   {id:'permissions',title:'Permissions',content:<><RoomPermissions roomId={channel}/><ChannelAdmin roomId={channel} onChanged={loadBootstrap}/></>},
