...
export function Workspace() {
    ...
    const loadBootstrap = useCallback(async () => { const b = await api('bootstrap'); setData(b); setPrefs({ ...defaultPrefs, ...b.preferences }); setProfileName(b.me.name); setWorkspaceName(b.workspace.name); if (!b.preview) {
        ...
        if(linkedServer){navigationGeneration.current++;setPrivateRoom('');setDetail(false);setView('channel');setTab('messages');setThread(null);setSelectedServer(linkedServer.id);setGuild(b.conversations.find((c:Conv)=>linkedServer.roomIds.includes(c.id))?.id||'');pendingRoom.current='';return b as Bootstrap;}
  ...
  {!privateMessage && !inThread && m.replies > 0 && <button className='thread-link' onClick={() => openThread(m)}><MessageCircle size={15}/>{m.replies} {m.replies === 1 ? 'reply' : 'replies'}<span>View thread</span><ChevronRight size={13}/></button>}
