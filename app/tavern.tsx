...
import { TextMediaSettings } from './text-media-settings';
...
function Workspace() {
 ...
 <KeyboardShortcuts currentRoomId={channel} userId={me?.id} enterToSend={textMedia.enterToSend} conversations={(data?.conversations||[]).map(c=>({id:c.id,unread:c.unread||Number(navigationPreferences().unread.includes(c.id)),muted:prefs.muted.includes(c.id)}))} onSelectRoom={select}/><CommandPalette onMessage={startDm} onSelectRoom={select} onSelectServer={chooseServer} onSettings={openSettings} onSearch={()=>setModal('search')} onCreate={!data?.preview&&(!currentServer||canEditCommunity(currentServer.id,'layout'))?()=>setModal('create'):undefined} onContacts={()=>openSettings('social')}/>
