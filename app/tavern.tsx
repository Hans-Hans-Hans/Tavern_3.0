import {PersonalServerMark,PersonalServerDialog} from './server-personalization';
import { joinVoiceChannel } from '@/lib/voice-channel-join';
import { StickerPackManager, StickerPicker } from './stickers';
import { type Sticker } from '@/lib/sticker-model';
function navigateWorkspace(hash:string){if(location.hash!==hash)history.pushState({tavern:true},'',location.pathname+location.search+hash);}
import { navigationScope, readConversationMemory, saveConversationMemory, rememberedConversation } from '@/lib/conversation-memory';
import { PersonalHome } from './personal-home';
import { experienceError } from '@/lib/experience-errors';
import { sectionConversations, sectionForConversation } from '@/lib/conversation-navigation';
...
import { canAssignMemberRoles, readRolePolicy } from '@/lib/roles';
...
import { VoiceSidebarParticipants, VoiceSidebarDock } from './voice-sidebar';
import { ChannelCreationForm } from './channel-creation';
import { ManagementSections } from './management-sections';
import { setNotificationFocus } from '@/lib/notifications';
import { matrixTyping, sendMatrixTyping } from '@/lib/matrix';
import { ProfileSettings, CommunityImage, CommunityAvatar, UserProfileCard, ChannelAppearanceSettings, ServerCustomization, ServerWelcome } from './community-settings';
...
import { ChannelAudienceSettings } from './channel-audience';
import { RoomRemovalSettings, RoomRemovalHistory } from './room-removal';
import { canEditConversationDetails } from '@/lib/channel-administration';
import { MobileServerNavigation } from './mobile-server-navigation';
...
import { ServerRoleName } from './role-identity';
import { accountArtworkOwner, isManagedAccount, requestApi } from '@/lib/api';
import { dmRequests, isDmRequest } from '@/lib/dm-requests';
import { canRestoreDirectListing } from '@/lib/dm-recovery';
import { showInDirectMessages } from '@/lib/matrix';
...
const ServerOverview = deferredPanel('Server overview', () => import('./server-overview').then(m => ({ default: m.ServerOverview })));
...
export function Workspace() {
    ...
    const lastServerSection = useRef('all');
    const restoredNavigation = useRef<object | null>(null);
    ...
    const [personalServer,setPersonalServer]=useState<{id:string;name:string}|null>(null);
    const [data, setData] = useState<Bootstrap | null>(null), [prefs, setPrefs] = useState<Prefs>(defaultPrefs), [channel, setGuild] = useState('general'), [view, setView] = useState(() => location.hash === '#home' ? 'home' : /^#view=(saved|mentions|threads|files|friends)$/.test(location.hash) ? location.hash.slice(6) : 'channel'), [tab, setTab] = useState('messages');
    ...
    const [serverSettingsSection, setServerSettingsSection] = useState('overview');
    const [detailSection, setDetailSection] = useState('overview');
    const [categoryRequest, setCategoryRequest] = useState({serverId:'', nonce:0});
    ...
    const [hasMore, setHasMore] = useState(false), [busy, setBusy] = useState(false), [email, setEmail] = useState(''), [invited, setInvited] = useState<string[]>([]), [profileName, setProfileName] = useState(''), [workspaceName, setWorkspaceName] = useState('');
    ...
    const [selectedServer, setSelectedServer] = useState(location.hash === '#view=friends' || new URLSearchParams(location.hash.slice(1)).get('section') === 'dms' ? 'dms' : 'all');
    const serverSelection = useRef(selectedServer);serverSelection.current=selectedServer;
    ...
    const managementClient=getMatrixClient(), managementIdentity=[managementClient,accountArtworkOwner(),managementClient?.getUserId(),managementClient?.getDeviceId(),managementClient?.getHomeserverUrl(),managementClient?.getRoom(channel),managementClient?.getRoom(currentServer?.id||'')];
    const managementScope=useRef({identity:managementIdentity,generation:0});
    if(managementIdentity.some((value,index)=>value!==managementScope.current.identity[index]))managementScope.current={identity:managementIdentity,generation:managementScope.current.generation+1};
    const navPreferences=navigationPreferences();
    const readAction=useMarkReadAction(markMatrixRoomsRead);
    const directSection = selectedServer === 'dms';
    const sectionRooms = sectionConversations(data?.conversations || [], data?.servers || [], selectedServer);
    const visibleChannels = sectionRooms.filter(c => c.kind !== 'dm').map(c=>({...c,manualUnread:navPreferences.unread.includes(c.id)}));
    const favoriteRooms = sectionRooms.filter(c => navPreferences.favorites.includes(c.id));
    function chooseServer(id: string) { if (id !== 'dms') lastServerSection.current = id; navigationGeneration.current++;setPrivateRoom('');setDetail(false);pendingRoom.current='';pendingEvent.current='';setSelectedServer(id);setView('channel');setTab('messages');setThread(null);setOpenMobile(false);const room=rememberedConversation(readConversationMemory(navigationScope(getMatrixClient())),id,data?.conversations||[],data?.servers||[]);setGuild(room?.id||'');navigateWorkspace(room?'#room='+encodeURIComponent(room.id):'#section='+encodeURIComponent(id)); }
    const me = data?.me;
    const active = data?.conversations.find(c => c.id === channel);
    const emptyDirect = directSection && !active && view==='channel';
    useEffect(() => {
      const client=getMatrixClient();
      if(!data?.preview && data?.me.id===client?.getUserId() && view==='channel' && client?.getRoom(channel)?.getMyMembership()==='join')
        saveConversationMemory(navigationScope(client),{section:selectedServer,roomId:channel});
    },[data?.me.id,channel,selectedServer,view]);
    useEffect(()=>{if(active?.kind==='dm'&&view==='channel'&&!privateRoom)setSelectedServer('dms');},[active?.id,active?.kind,view,privateRoom]);
    ...
    const loadBootstrap = useCallback(async () => { const owner=getMatrixClient(),account=accountArtworkOwner(); const b = await api('bootstrap'); if(getMatrixClient()!==owner||accountArtworkOwner()!==account)return b as Bootstrap; setData(b); setPrefs({ ...defaultPrefs, ...b.preferences }); setProfileName(b.me.name); setWorkspaceName(b.workspace.name); if(!b.preview){
            if(owner && restoredNavigation.current!==account){
              restoredNavigation.current=account;
              if(!pendingRoom.current && (!location.hash || location.hash==='#home')){
                const memory=readConversationMemory(navigationScope(owner));
                const section=['all','dms'].includes(memory.section)||b.servers?.some((server:{id:string})=>server.id===memory.section)?memory.section:'all';
                serverSelection.current=section;setSelectedServer(section);if(section!=='dms')lastServerSection.current=section;
                const remembered=rememberedConversation(memory,section,b.conversations,b.servers||[]);
                setGuild(remembered?.id||'');
                if(memory.landing==='home'||location.hash==='#home'){setView('home');history.replaceState(null,'',location.pathname+location.search+'#home');}else if(remembered){history.replaceState(null,'',location.pathname+location.search+'#room='+encodeURIComponent(remembered.id));}
        ...
        if (linked && b.conversations.some((c: Conv) => c.id === linked)) {
            ...
            setSelectedServer(current => sectionForConversation(b.conversations,b.servers||[],current,linked));
            pendingRoom.current = '';
            if(pendingEvent.current){const eventId=pendingEvent.current;pendingEvent.current='';openMessage(linked,eventId);}
        }
        else {
            setGuild(current => b.conversations.some((c: Conv) => c.id === current) ? current : sectionConversations(b.conversations,b.servers||[],serverSelection.current)[0]?.id || '');
    ...
    const loadMessages = useCallback(async (silent = false) => { const g = ++generation.current; if (!silent)
        setLoading(true); try {
        if(view==='home'||view==='friends'||view==='channel'&&!channel){setMessages([]);setHasMore(false);setError('');return;}
    ...
    useEffect(()=>{const follow=()=>{const params=new URLSearchParams(location.hash.slice(1)),requested=params.get('view');
      if(location.hash==='#home'||!location.hash||requested&&['saved','mentions','threads','files','friends'].includes(requested)){navigationGeneration.current++;setPrivateRoom('');setDetail(false);setView(requested||'home');if(requested==='friends')setSelectedServer('dms');setThread(null);setOpenMobile(false);return;}
      const section=params.get('section');if(section){const client=getMatrixClient(),current=data?.servers?.some(server=>server.id===section)||['all','dms'].includes(section)?section:'all';setSelectedServer(current);setGuild(rememberedConversation(readConversationMemory(navigationScope(client)),current,data?.conversations||[],data?.servers||[])?.id||'');setView('channel');setOpenMobile(false);return;}
      const link=parseConversationLink(location.hash);if(!link.roomId)return;pendingRoom.current=link.roomId;pendingEvent.current=link.eventId;void loadBootstrap().catch(e=>toast.error(e.message));};window.addEventListener('hashchange',follow);return()=>window.removeEventListener('hashchange',follow);},[loadBootstrap,data]);
    ...
    function select(id: string) {
        ...
        navigationGeneration.current++;pendingRoom.current='';pendingEvent.current='';setPrivateRoom('');setPrivateFocus(null);setDetail(false);setDetailSection('overview');setFocusMessage(null);setUnreadStart(null);setThread(null);setOpenMobile(false);
        ...
        setSelectedServer(current=>sectionForConversation(data?.conversations||[],data?.servers||[],current,id));setGuild(id);setView('channel');setTab('messages');navigateWorkspace('#room='+encodeURIComponent(id));
    }
    function selectChannel(id: string) {
        select(id);
        if (data?.preview || readChannelPolicy(id).kind !== 'voice') return;
        const generation = navigationGeneration.current;
        void joinVoiceChannel(id, () => navigationGeneration.current === generation).catch(error => {
            if (navigationGeneration.current === generation) toast.error(error.message);
        });
    }
    ...
    const go = (v: string) => { navigationGeneration.current++;setPrivateRoom('');setDetail(false);setView(v); if(v==='friends')setSelectedServer('dms'); setThread(null); setOpenMobile(false);navigateWorkspace(v==='channel'?'#room='+encodeURIComponent(channel):v==='home'?'#home':'#view='+v); };
    ...
    function openSettings(t = 'appearance') { setOpenMobile(false); setSettingsTab(t); setModal('settings'); }
    ...
    async function startDm(member: string) {
      const owner=getMatrixClient(),account=accountArtworkOwner(),navigation=navigationGeneration.current;
      const ownsAccount=()=>getMatrixClient()===owner&&accountArtworkOwner()===account;
      const current=()=>ownsAccount()&&navigationGeneration.current===navigation;
      setBusy(true); try {
        const r = await api('create', { kind: 'dm', name: 'Direct message', members: [member] });
        if(!current())return;
        await loadBootstrap();
        if(!current())return;
        select(r.id);setSelectedServer('dms');
        setModal('');return r.id as string;
    }
    catch (e) {
        if(current())toast.error((e as Error).message);
    }
    finally {
        if(ownsAccount())setBusy(false);
    } }
    ...
    function roomActions(c:Conv):ContextAction[]{const room=getMatrixClient()?.getRoom(c.id),uid=getMatrixClient()?.getUserId();const navPrefs=navigationPreferences();return [
      ...
      {label:'Show in Direct Messages',visible:c.kind!=='dm'&&canRestoreDirectListing(room),run:async()=>{await showInDirectMessages(c.id);await loadBootstrap();toast.success('Conversation added to Direct Messages');}},
      ...
      {label:'Notification settings',run:()=>{select(c.id);setDetailSection('notifications');setDetail(true);}},
      {label:'Copy channel link',run:()=>copyText(location.origin+'/#room='+encodeURIComponent(c.id))},
      {label:'Copy channel ID',run:()=>copyText(c.id)},
      {label:'Edit channel & permissions',visible:canEditConversationDetails(c.id),run:()=>{select(c.id);setDetail(true);}},
      {label:'Delete channel',danger:true,visible:c.kind!=='dm'&&isManagedAccount()&&!!uid&&!!data?.servers?.some(server=>server.roomIds.includes(c.id)&&readRolePolicy(server.id)?.owner===uid),run:()=>{select(c.id);setDetailSection('deletion');setDetail(true);}},
    ...
    function openServerSettings(serverId:string,section='overview'){chooseServer(serverId);setServerSettingsSection(section);setModal('serverSettings');}
    function serverActions(server:{id:string;name:string}):ContextAction[]{const room=getMatrixClient()?.getRoom(server.id),prefs=navigationPreferences();return [{label:'Personal server color',run:()=>setPersonalServer({id:server.id,name:server.name})},{label:'Open server',run:()=>chooseServer(server.id)},{label:'Welcome and rules',run:()=>{chooseServer(server.id);setModal('serverWelcome');}},{label:prefs.favorites.includes(server.id)?'Remove favorite':'Favorite server',run:()=>setNavigationFlag(server.id,'favorites',!prefs.favorites.includes(server.id))},{label:'Mark server read',run:()=>readAction.run((room?.currentState.getStateEvents('m.space.child')||[]).filter(e=>Array.isArray(e.getContent().via)&&e.getContent().via.length>0).map(e=>e.getStateKey()!).filter(Boolean))},{label:'Create channel',visible:canEditCommunity(server.id,'layout'),run:()=>{chooseServer(server.id);setCreationCategory('');setModal('create');}},{label:'Create category',visible:canEditCommunity(server.id,'layout'),run:()=>{chooseServer(server.id);setCategoryRequest(old=>({serverId:server.id,nonce:old.nonce+1}));}},{label:'Server settings',visible:canEditCommunity(server.id,'layout'),run:()=>openServerSettings(server.id)},{label:'Server notification settings',run:()=>openServerSettings(server.id,'notifications')},{label:'Invitation privacy',visible:isManagedAccount(),run:()=>{chooseServer(server.id);openSettings('social');}},{label:'Server profile',run:()=>{chooseServer(server.id);openSettings('profile');}},{label:'Invite people',visible:canInviteToRoom(server.id),run:()=>{chooseServer(server.id);setModal('inviteServer');}},{label:'Manage integrations',visible:roomWebhookLocations(server.id).length>0,run:()=>{chooseServer(server.id);setModal('serverIntegrations');}},{label:'Review reports',visible:roomReportLocations(server.id).length>0,run:()=>{chooseServer(server.id);setModal('serverReports');}},{label:'Audit log',visible:canViewServerAudit(server.id),run:()=>{chooseServer(server.id);setModal('serverAudit');}},{label:'Member warnings',visible:isManagedAccount()&&canBulkRedact(server.id),run:()=>{chooseServer(server.id);setModal('serverWarnings');}},{label:'Copy server link',run:()=>copyText(location.origin+'/#server='+encodeURIComponent(server.id))},{label:'Copy server ID',run:()=>copyText(server.id)},{label:'Report server',visible:isManagedAccount(),run:()=>setReportTarget({kind:'server',roomId:server.id})},{label:'Leave server',danger:true,run:()=>setConfirmAction({title:'Leave '+server.name+'?',description:'This removes your server membership. Leave individual channels separately if you also want to leave those conversations.',run:async()=>{await getMatrixClient()!.leave(server.id);await loadBootstrap();chooseServer('all');}})}];}
  ...
  {!privateMessage && !inThread && <DraftThreadLink roomId={m.conversation_id} rootId={m.id} replies={m.replies} onOpen={() => openThread(m)}/>}
   ...
   <DropdownMenu><DropdownMenuTrigger asChild><button className='icon-button' aria-label='More message actions'><MoreHorizontal size={18}/></button></DropdownMenuTrigger><DropdownMenuContent align='end'>{permissions.pin&&<DropdownMenuItem onSelect={() => act('pin', { id: m.id }).catch(() => { })}><Pin />{m.pinned ? 'Unpin message' : label("Pin to Guild")}</DropdownMenuItem>}{(permissions.edit||permissions.delete) && <><DropdownMenuSeparator />{permissions.edit&&<DropdownMenuItem onSelect={() => { setEditing(m); setEditBody(m.body); }}><Pencil />Edit message</DropdownMenuItem>}{permissions.delete&&<DropdownMenuItem className='danger-text' onSelect={() => setDeleting(m)}><Trash2 />Delete message</DropdownMenuItem>}</>}</DropdownMenuContent></DropdownMenu>
    ...
    const heading = emptyDirect ? 'Direct messages' : view === 'channel' ? name : ({ friends: 'Friends', home: 'Home', mentions: 'Mentions', threads: 'Threads', saved: 'Saved for later', files: 'Shared files' } as any)[view] || name;
    function browseSection(id: string) { if (id !== selectedServer) chooseServer(id); else if (view !== 'channel') go('channel'); if (isMobile) setOpenMobile(true); }
    const navigationRail = <aside className='workspace-rail' aria-label='Tavern navigation'><button className='brand-mark' aria-label='Tavern home' aria-current={view==='home'?'page':undefined} onClick={() => {go('home');}}><Beer size={27} strokeWidth={2}/></button><div className='rail-divider'/><button className={'workspace-icon ' + (selectedServer === 'all' ? 'selected' : '')} onClick={() => browseSection('all')} title='All channels' aria-label='All channels' >T<span /></button><button className={'workspace-icon '+(directSection?'selected':'')} aria-label='Direct messages' aria-current={directSection?'page':undefined} title='Direct messages' onClick={()=>browseSection('dms')}><MessageCircle size={24}/></button><ServerNavigation servers={data?.servers||[]} readState={{roomIds:(data?.conversations||[]).map(room=>room.id),muted:prefs.muted,focus:prefs.focus}} active={selectedServer} onSelectServer={browseSection} renderServer={(server,button,organizationActions)=><ActionMenu actions={[...serverActions(server).filter(action=>!['Favorite server','Remove favorite'].includes(action.label)), ...organizationActions]}><PersonalServerMark serverId={server.id}>{button}</PersonalServerMark></ActionMenu>}/><IconButton label={'Create ' + terms.server} disabled={!!data?.preview} onClick={() => setModal('server')}><Plus size={20}/></IconButton><IconButton label='Tavern settings' onClick={() => openSettings('workspace')}><Settings size={20}/></IconButton><div className='rail-spacer'/><IconButton label='About Tavern' onClick={() => openSettings('about')}><CircleHelp size={20}/></IconButton><button className='rail-profile' aria-label='Your profile' onClick={() => openSettings('profile')}><Avatar name={me?.name || 'You'} size='small'/></button></aside>;
    return <>
 {personalServer&&<PersonalServerDialog key={personalServer.id} server={personalServer} onClose={()=>setPersonalServer(null)}/>}
 {!isMobile && navigationRail}
 <Sidebar collapsible='offcanvas' className='channel-sidebar' mobileRail={navigationRail}>
  <SidebarHeader className='workspace-header'>{isMobile && <button className='mobile-nav-close' aria-label='Close navigation' onClick={() => setOpenMobile(false)}><X size={20}/></button>}<DropdownMenu><DropdownMenuTrigger asChild><button className='workspace-select'><span><strong>{directSection ? 'Direct messages' : currentServer?.name || data?.workspace.name || 'Tavern'}</strong><small><span className='matrix-wordmark'>[m]</span> Self-hosted · Matrix</small></span><ChevronDown size={17}/></button></DropdownMenuTrigger><DropdownMenuContent className='workspace-menu' align='start'>{currentServer&&serverActions(currentServer).filter(action=>action.visible!==false&&action.label!=='Open server').map(action=><DropdownMenuItem key={action.label} onSelect={()=>{try{Promise.resolve(action.run()).catch(error=>toast.error(experienceError(error).message));}catch(error){toast.error((error as Error).message);}}}>{action.label}</DropdownMenuItem>)}<DropdownMenuSeparator/><DropdownMenuItem onSelect={()=>chooseServer('all')}>All channels</DropdownMenuItem><DropdownMenuItem onSelect={()=>chooseServer('dms')}>Direct messages</DropdownMenuItem><DropdownMenuItem disabled={!!data?.preview} onSelect={()=>setModal('server')}>Create {terms.server}</DropdownMenuItem><DropdownMenuItem onSelect={()=>openSettings('workspace')}><Settings/>Tavern settings</DropdownMenuItem><DropdownMenuItem onSelect={()=>openSettings('privacy')}><ShieldCheck/>Privacy &amp; security</DropdownMenuItem></DropdownMenuContent></DropdownMenu><MobileServerNavigation servers={data?.servers||[]} readState={{roomIds:(data?.conversations||[]).map(room=>room.id),muted:prefs.muted,focus:prefs.focus}} active={selectedServer} onSelectServer={browseSection} serverActions={serverActions}/></SidebarHeader>
  <SidebarContent className='navigation-content'><nav className='quick-nav' aria-label='Messages'>{directSection&&<button className={view==='friends'?'active':''} onClick={()=>go('friends')}><Users size={18}/><span>Friends</span></button>}<button className={view === 'mentions' ? 'active' : ''} onClick={() => go('mentions')}><AtSign size={18}/><span>Mentions</span></button><button className={view === 'threads' ? 'active' : ''} onClick={() => go('threads')}><MessageSquare size={18}/><span>Threads</span></button><button className={view === 'saved' ? 'active' : ''} onClick={() => go('saved')}><Bookmark size={18}/><span>Saved for later</span></button><button onClick={()=>setModal('dmRequests')}><MessageCircle size={18}/><span>Message requests ({dmRequests().length})</span></button><button onClick={()=>setModal('privateDiscussions')}><LockKeyhole size={18}/><span>Private discussions</span></button></nav>
   {favoriteRooms.length>0&&<div className='channel-section'><div className='nav-section-label'>FAVORITES</div>{favoriteRooms.map(c=><ActionMenu key={c.id} actions={roomActions(c)}><button className={'channel-link '+(channel===c.id?'active':'')} onClick={()=>select(c.id)}><Star size={16}/><span>{convName(c)}</span><DraftIndicator roomId={c.id} label={'Draft in '+convName(c)}/></button></ActionMenu>)}</div>}{!directSection&&<div className='channel-section'><div className='nav-section-label'><span><ChevronDown size={13}/>{label(" GUILDS")}</span><IconButton label={label("Create a Guild")} onClick={() => {setCreationCategory('');setModal('create');}}><Plus size={15}/></IconButton></div><ChannelNavigation serverId={currentServer?.id} channels={visibleChannels||[]} active={view==='channel'?channel:''} muted={prefs.muted} focus={prefs.focus} onSelect={selectChannel} createCategoryRequest={categoryRequest.serverId===currentServer?.id?categoryRequest.nonce:0} onCreateChannel={({category})=>{setCreationCategory(category);setModal('create');}} onEditChannel={id=>{select(id);setDetail(true);}} onInviteChannel={id=>{select(id);setModal('invite');}} renderParticipants={id=><VoiceSidebarParticipants roomId={id}/>} renderChannel={(c,button,extraActions)=><ActionMenu key={c.id} actions={[...roomActions(data!.conversations.find(room=>room.id===c.id)!),...extraActions]}><div className='channel-nav-row'>{button}<DraftIndicator roomId={c.id} label={'Draft in '+c.name}/></div></ActionMenu>}/><button className='add-channel' onClick={() => {setCreationCategory('');setModal('create');}}><Plus size={16}/>{label(" Add a Guild")}</button></div>}
   {currentServer&&<AvailableChannels key={currentServer.id} serverId={currentServer.id} onOpen={async id=>{const owner=getMatrixClient(),account=accountArtworkOwner(),generation=navigationGeneration.current;await loadBootstrap();if(owner===getMatrixClient()&&account===accountArtworkOwner()&&generation===navigationGeneration.current)select(id);}}/>}{directSection&&<div className='channel-section dm-section'><div className='nav-section-label'><span><ChevronDown size={13}/> DIRECT MESSAGES</span><IconButton label='New direct message' onClick={() => setModal('dm')}><Plus size={15}/></IconButton></div>{data?.conversations.filter(c => c.kind === 'dm').map(c => <ActionMenu key={c.id} actions={roomActions(c)}><button key={c.id} className={'channel-link dm-link ' + (channel === c.id && view === 'channel' ? 'active' : '')} onClick={() => select(c.id)}><Avatar name={convName(c)} size='tiny'/><span>{convName(c)}</span><DraftIndicator roomId={c.id} label={'Draft in '+convName(c)}/><ReadStateBadges unread={c.unread} mentions={c.mentions} manual={navPreferences.unread.includes(c.id)} muted={prefs.muted.includes(c.id)} focus={prefs.focus}/></button></ActionMenu>)}{!data?.conversations.some(c => c.kind === 'dm') && <button className='channel-link dm-link' onClick={() => me && startDm(me.id)}><Avatar name={me?.name || 'You'} size='tiny'/><span>Notes to self</span><span className='small-note'>you</span></button>}<button className='add-channel' onClick={() => setModal('dm')}><Plus size={16}/> New message</button></div>}
 ...
 <main className='main-workspace'>
  ...
  <div className='conversation-layout'><section className='conversation-main'>
   ...
   {view === 'friends' ? <div className='friends-workspace'><SocialPanel onMessage={startDm}/></div> : view === 'home' ? <PersonalHome rooms={(data?.conversations||[]).map(room=>({...room,name:convName(room)}))} onSelect={select} onMessage={openMessage} onView={go} onStart={()=>setModal('dm')}/> : emptyDirect ? <div className='empty-state'><MessageCircle size={36}/><h2>Your direct messages</h2><p>Choose someone to start a private conversation. Individual and group messages will appear here.</p><button className='primary-button' onClick={()=>setModal('dm')}>Start a direct message</button></div>:view==='channel'&&voiceMode?<VoiceChannel key={channel} roomId={channel} name={heading} disabled={!!data?.preview} onProfile={id=>openProfile(id,channel)}/>:view==='channel'&&tab==='messages'&&forumMode?<ForumChannel key={channel} roomId={channel} serverId={currentServer?.id} messages={messages} onOpen={openThread} onSent={()=>loadMessages(true)} onProfile={id=>openProfile(id,channel)} hasMore={hasMore} loadingHistory={busy} onLoadHistory={loadOlder}/>:view==='channel'&&tab==='work'?<CollaborationBoard key={channel} roomId={channel} onRemind={eventId=>setReminder({roomId:channel,eventId})}/>:<div className='message-scroll' ref={scroll} onScroll={() => { if (scroll.current)
    ...
    {loading && messages.length === 0 ? <div className='loading-messages'><Loader2 size={23} className='spin'/><span>Opening your conversations…</span></div> : <>
     {view === 'channel' && tab === 'messages' && messages.length===0 && !hasMore && <div className='channel-intro'><span className='intro-hash'>{active?.kind === 'dm' ? <MessageCircle size={30}/> : active?.kind === 'private' ? <LockKeyhole size={30}/> : <Hash size={36}/>}</span><h2>{!active && !data?.preview ? label("Your Tavern is ready") : active?.kind === 'dm' ? name : `Welcome to #${name}`}</h2><p>{!active && !data?.preview ? 'Create your first encrypted room to start a conversation.' : active?.kind === 'dm' ? (name === 'Notes to self' ? 'A quiet spot for thoughts, links, and things to remember.' : 'This is the beginning of your private conversation.') : active?.description || 'This is the beginning of something good.'}</p><div className='intro-meta'><span><LockKeyhole size={13}/>{active?.kind === 'private' ? label("Private Guild") : active?.kind === 'dm' ? 'Private conversation' : active?.private ? 'Invite-only room' : 'Room members'}</span><span>·</span><button onClick={() => setDetail(true)}>View details <ArrowRight size={13}/></button></div></div>}
     ...
     {view === 'channel' && tab === 'messages' && messages.length===0 && !hasMore && <div className='channel-start-actions'>
       {active ? <button className='primary-button' onClick={()=>document.querySelector<HTMLTextAreaElement>('textarea[aria-label^="Message "]')?.focus()}>Write the first message</button> : <button className='primary-button' onClick={()=>currentServer&&!canEditCommunity(currentServer.id,'layout')?setModal('conversation'):setModal(currentServer?'create':'server')}>{currentServer?(canEditCommunity(currentServer.id,'layout')?'Create a channel':'Message someone'):'Create a server'}</button>}
       <button className='secondary-button' onClick={openWelcomeTour}>Getting started</button>
     </div>}
 ...
 <KeyboardShortcuts currentRoomId={privateRoom||channel} userId={me?.id} enterToSend={textMedia.enterToSend} conversations={(data?.conversations||[]).map(c=>({id:c.id,unread:c.unread||Number(navigationPreferences().unread.includes(c.id)),muted:prefs.muted.includes(c.id)}))} onSelectRoom={select} onMarkAllRead={readAction.available&&!data?.preview?()=>void readAction.run():undefined}/><CommandPalette onMessage={startDm} onSelectRoom={select} onSelectServer={chooseServer} onSettings={openSettings} onSearch={query=>{setSearch(query||'');setModal('search');}} onCreate={!directSection&&!data?.preview&&(!currentServer||canEditCommunity(currentServer.id,'layout'))?()=>setModal('create'):undefined} onContacts={()=>go('friends')}/>
 {modal === 'server' && <ServerDialog open naming={prefs.naming} onClose={() => setModal('')} onCreated={async (id, roomId, isCurrent) => { const navigation=navigationGeneration.current; await loadBootstrap(); if(!isCurrent()||navigationGeneration.current!==navigation)return; setSelectedServer(id); setGuild(roomId || ''); setView('channel'); }}/>}
 <Dialog open={['create', 'dm', 'invite', 'members', 'settings', 'search', 'connect', 'roomInvites', 'inviteServer'].includes(modal)} onOpenChange={v => { if (!v && !connecting)
  ...
  {modal === 'create' && !data?.preview && <ChannelCreationForm initialCategoryId={creationCategory} serverId={currentServer?.id} members={data?.members || []} policyEnabled={rolePolicyEnabled} callsEnabled={callsEnabled} onCreated={async (id,isCurrent) => { const navigation=navigationGeneration.current; await loadBootstrap(); if(!isCurrent()||navigationGeneration.current!==navigation)return; select(id); setModal(''); }}/>}
  {modal === 'dm' && <div className='dialog-member-list'><GroupMessage members={data?.members.filter(m=>m.id!==me?.id)||[]} onCreated={async id=>{await loadBootstrap();select(id);setSelectedServer('dms');setModal('');}}/>{data?.members.map(m => <button key={m.id} disabled={busy} onClick={() => startDm(m.id)}><Avatar name={m.name}/><span><strong>{m.id === me?.id ? 'Notes to self' : m.name}</strong><small>{m.id === me?.id ? 'Just for you' : 'Send a private message'}</small></span><ArrowRight size={17}/></button>)}</div>}
 ...
 <Dialog open={modal==='serverSettings'} onOpenChange={open=>{if(!open)setModal('');}}><DialogContent className='tavern-dialog settings-dialog'><DialogHeader><DialogTitle>Server settings</DialogTitle><DialogDescription>Your community, channels, roles and moderation in one place.</DialogDescription></DialogHeader>{currentServer&&<ManagementSections key={currentServer.id+':'+managementScope.current.generation} label='Server settings sections' initialSection={serverSettingsSection} sections={[
   {id:'overview',title:'Overview',content:<ServerOverview serverId={currentServer.id} onCreateChannel={()=>{setCreationCategory('');setModal('create');}} onInvite={canInviteToRoom(currentServer.id)?()=>setModal('inviteServer'):undefined}/>},
   {id:'organization',title:'Server & channels',content:<ServerCustomization serverId={currentServer.id} onChanged={loadBootstrap} onOrganize={()=>{setModal('');setCategoryRequest(old=>({serverId:currentServer.id,nonce:old.nonce+1}));}}/>},
   {id:'welcome',title:'Welcome',content:<ServerOnboardingSettings serverId={currentServer.id}/>},
   {id:'roles',title:'Roles',content:<ServerRoles serverId={currentServer.id} enabled={rolePolicyEnabled} onChanged={loadBootstrap}/>},
   {id:'emoji',title:'Emoji',content:<ServerEmojiManager serverId={currentServer.id}/>},
   {id:'stickers',title:'Stickers',content:<StickerPackManager key={currentServer.id} serverId={currentServer.id}/>},
   {id:'notifications',title:'Notifications',content:<NotificationSettings serverId={currentServer.id}/>},
   ...(roomWebhookLocations(currentServer.id).length?[{id:'integrations',title:'Integrations',content:<RoomIntegrations roomId={currentServer.id} inline/>}]:[]),
   ...(roomReportLocations(currentServer.id).length?[{id:'reports',title:'Reports',content:<RoomReportReview roomId={currentServer.id}/>}]:[]),
   ...(canViewServerAudit(currentServer.id)?[{id:'audit',title:'Audit log',content:<ServerAudit serverId={currentServer.id}/>}]:[]),
   ...(isManagedAccount()&&canBulkRedact(currentServer.id)?[{id:'warnings',title:'Member warnings',content:<ServerWarnings roomId={currentServer.id}/>}]:[]),
   ...(isManagedAccount()&&readRolePolicy(currentServer.id)?.owner===getMatrixClient()?.getUserId()?[{id:'deletion',title:'Delete server',content:<RoomRemovalSettings roomId={currentServer.id} onChanged={loadBootstrap}/>}]:[]),
   ...(isManagedAccount()?[{id:'invitations',title:'Invitations',content:<InviteManager roomId={currentServer.id}/>}]:[])
 ]}/>}</DialogContent></Dialog>
 ...
 <Sheet open={detail&&!privateRoom&&!thread} onOpenChange={setDetail}><SheetContent className='detail-sheet'><SheetHeader><SheetTitle>{voiceMode?<Headphones/>:active?.kind==='private'?<LockKeyhole/>:<Hash/>}{name}</SheetTitle><SheetDescription>Channel information and management</SheetDescription></SheetHeader>
  <div className='detail-content'><ManagementSections key={channel+':'+managementScope.current.generation} label='Channel settings sections' initialSection={detailSection} sections={[
   {id:'overview',title:'Overview',content:<><h3>Description</h3><p>{active?.description||'A private conversation for the people here.'}</p><h3>Who can see this?</h3><div className='notice'><LockKeyhole size={20}/><p>Room membership controls access to this conversation and its encrypted history.</p></div><div className='setting-row'><span><strong>Mute this conversation</strong><small>Hide unread badges in the sidebar.</small></span><Switch aria-label='Mute conversation' checked={muted} onCheckedChange={()=>updatePrefs({muted:muted?prefs.muted.filter(c=>c!==channel):[...prefs.muted,channel]})}/></div><ChannelAdmin section='details' roomId={channel} onChanged={loadBootstrap}/><ChannelAppearanceSettings roomId={channel} onChanged={loadBootstrap}/><PrivateThreadLauncher sourceRoomId={channel} onOpen={openPrivateDiscussion}/></>},
   {id:'notifications',title:'Notifications',content:<NotificationSettings roomId={channel} serverId={currentServer?.id}/>},
   {id:'behavior',title:'Behavior',content:<><ChannelPolicySettings roomId={channel} enabled={rolePolicyEnabled} onChanged={loadBootstrap}/><EnableThreadSettings roomId={channel} enabled={rolePolicyEnabled}/></>},
   {id:'permissions',title:'Permissions',content:<>{data?.servers?.filter(server=>server.roomIds.includes(channel)).map(server=><div key={server.id+':'+channel}><ChannelAudienceSettings serverId={server.id} roomId={channel} onChanged={loadBootstrap}/><ServerRoles serverId={server.id} channelId={channel} enabled={rolePolicyEnabled} onChanged={loadBootstrap}/></div>)}<RoomPermissions roomId={channel}/></>},
   {id:'members',title:'Members',content:<><h3>Channel members</h3>{channelMembers.map(m=><button className='member-row' key={m.id} onClick={()=>openProfile(m.id,channel)}><CommunityAvatar roomId={channel} userId={m.id} size={32} fallback={m.name}/><span><strong>{m.name}</strong><small>{m.role}</small></span></button>)}</>},
   ...(isManagedAccount()?[{id:'invitations',title:'Invitations',content:<InviteManager roomId={channel}/>}]:[]),
   {id:'moderation',title:'Moderation',content:<><ChannelAdmin section='members' roomId={channel} onChanged={loadBootstrap}/><RoomReportReview roomId={channel}/><TemporaryBans roomId={channel}/><ServerWarnings roomId={channel}/><BulkModeration roomId={channel} messages={messages.filter(m=>m.conversation_id===channel)} onChanged={()=>loadMessages(true)}/><ModerationActions roomId={channel} enabled={rolePolicyEnabled} onChanged={loadBootstrap}/></>},
   {id:'integrations',title:'Integrations',content:<RoomIntegrations roomId={channel}/>},
   ...(isManagedAccount()&&data?.servers?.some(server=>server.roomIds.includes(channel)&&readRolePolicy(server.id)?.owner===getMatrixClient()?.getUserId())?[{id:'deletion',title:'Delete channel',content:<RoomRemovalSettings roomId={channel} onChanged={loadBootstrap}/>}]:[])
    ...
    setUploading(true);const controller=new AbortController();uploadAbort.current=controller; for (const f of Array.from(list)) {
        ...
        try {
            ...
            if(!controller.signal.aborted&&current()){
                if(experienceError(e,'upload').status===413){toast.error(experienceError(e,'upload').message+' Your message draft is kept.');setFailedUploads(old=>[...old,f]);continue;}
