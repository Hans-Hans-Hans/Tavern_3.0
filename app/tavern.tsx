import { TemporaryBans } from './temporary-bans';
...
function Workspace() {
    ...
    function chooseServer(id: string) { pendingRoom.current='';pendingEvent.current='';setSelectedServer(id); setView('channel'); setThread(null); const server = data?.servers?.find(s => s.id === id); setGuild(server ? data?.conversations.find(c => server.roomIds.includes(c.id))?.id || '' : data?.conversations[0]?.id || 'general'); }
    const me = data?.me;
    ...
    const loadBootstrap = useCallback(async () => { const b = await api('bootstrap'); setData(b); setPrefs({ ...defaultPrefs, ...b.preferences }); setProfileName(b.me.name); setWorkspaceName(b.workspace.name); if (!b.preview) {
        ...
        if(linkedServer){setView('channel');setTab('messages');setThread(null);setSelectedServer(linkedServer.id);setGuild(b.conversations.find((c:Conv)=>linkedServer.roomIds.includes(c.id))?.id||'');pendingRoom.current='';return b as Bootstrap;}
        if (linked && b.conversations.some((c: Conv) => c.id === linked)) {
            setGuild(linked); setView('channel'); setTab('messages'); setThread(null);
    ...
    const select = (id: string) => { pendingRoom.current='';pendingEvent.current='';setFocusMessage(null);setUnreadStart(null); if(currentServer&&!currentServer.roomIds.includes(id))setSelectedServer('all'); setGuild(id); setView('channel'); setTab('messages'); setThread(null); setOpenMobile(false); };
