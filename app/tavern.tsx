...
export function Workspace() {
    ...
    const [data, setData] = useState<Bootstrap | null>(null), [prefs, setPrefs] = useState<Prefs>(defaultPrefs), [channel, setGuild] = useState('general'), [view, setView] = useState(() => location.hash === '#home' ? 'home' : /^#view=(saved|mentions|threads|files)$/.test(location.hash) ? location.hash.slice(6) : 'channel'), [tab, setTab] = useState('messages');
    ...
    const [serverSettingsSection, setServerSettingsSection] = useState('organization');
    ...
    const [selectedServer, setSelectedServer] = useState(new URLSearchParams(location.hash.slice(1)).get('section') === 'dms' ? 'dms' : 'all');
    ...
    const loadMessages = useCallback(async (silent = false) => { const g = ++generation.current; if (!silent)
        setLoading(true); try {
        if(view==='home'||view==='channel'&&!channel){setMessages([]);setHasMore(false);setError('');return;}
    ...
    useEffect(()=>{const follow=()=>{const params=new URLSearchParams(location.hash.slice(1)),requested=params.get('view');
      if(location.hash==='#home'||!location.hash||requested&&['saved','mentions','threads','files'].includes(requested)){navigationGeneration.current++;setPrivateRoom('');setDetail(false);setView(requested||'home');setThread(null);setOpenMobile(false);return;}
    ...
    const go = (v: string) => { navigationGeneration.current++;setPrivateRoom('');setDetail(false);setView(v); setThread(null); setOpenMobile(false);navigateWorkspace(v==='channel'?'#room='+encodeURIComponent(channel):v==='home'?'#home':'#view='+v); };
    ...
    function openServerSettings(serverId:string,section='organization'){chooseServer(serverId);setServerSettingsSection(section);setModal('serverSettings');}
  ...
  {!privateMessage && !inThread && <DraftThreadLink roomId={m.conversation_id} rootId={m.id} replies={m.replies} onOpen={() => openThread(m)}/>}
   ...
   <DropdownMenu><DropdownMenuTrigger asChild><button className='icon-button' aria-label='More message actions'><MoreHorizontal size={18}/></button></DropdownMenuTrigger><DropdownMenuContent align='end'>{permissions.pin&&<DropdownMenuItem onSelect={() => act('pin', { id: m.id }).catch(() => { })}><Pin />{m.pinned ? 'Unpin message' : label("Pin to Guild")}</DropdownMenuItem>}{(permissions.edit||permissions.delete) && <><DropdownMenuSeparator />{permissions.edit&&<DropdownMenuItem onSelect={() => { setEditing(m); setEditBody(m.body); }}><Pencil />Edit message</DropdownMenuItem>}{permissions.delete&&<DropdownMenuItem className='danger-text' onSelect={() => setDeleting(m)}><Trash2 />Delete message</DropdownMenuItem>}</>}</DropdownMenuContent></DropdownMenu>
    ...
    const heading = emptyDirect ? 'Direct messages' : view === 'channel' ? name : ({ home: 'Home', mentions: 'Mentions', threads: 'Threads', saved: 'Saved for later', files: 'Shared files' } as any)[view] || name;
 ...
 <KeyboardShortcuts currentRoomId={privateRoom||channel} userId={me?.id} enterToSend={textMedia.enterToSend} conversations={(data?.conversations||[]).map(c=>({id:c.id,unread:c.unread||Number(navigationPreferences().unread.includes(c.id)),muted:prefs.muted.includes(c.id)}))} onSelectRoom={select} onMarkAllRead={readAction.available&&!data?.preview?()=>void readAction.run():undefined}/><CommandPalette onMessage={startDm} onSelectRoom={select} onSelectServer={chooseServer} onSettings={openSettings} onSearch={query=>{setSearch(query||'');setModal('search');}} onCreate={!directSection&&!data?.preview&&(!currentServer||canEditCommunity(currentServer.id,'layout'))?()=>setModal('create'):undefined} onContacts={()=>openSettings('social')}/>
 ...
 <Dialog open={modal==='serverSettings'} onOpenChange={open=>{if(!open)setModal('');}}><DialogContent className='tavern-dialog settings-dialog'><DialogHeader><DialogTitle>Server settings</DialogTitle><DialogDescription>Appearance, welcome screen, and channel organization.</DialogDescription></DialogHeader>{currentServer&&<ManagementSections key={currentServer.id+':'+managementScope.current.generation} label='Server settings sections' initialSection={serverSettingsSection} sections={[
