...
import { matrixApi as api, connectMatrix, restoreMatrixSession, disconnectMatrix, matrixStatus, onMatrixUpdate, uploadMatrixFile, discardMatrixFile, downloadMatrixFile, exportMatrixMessages, exportEncryptionKeys, importEncryptionKeys, getMatrixClient, resolveMatrixMessage, markMatrixRoomsRead } from '@/lib/matrix';
...
export function Workspace() {
 ...
 <KeyboardShortcuts currentRoomId={privateRoom||channel} userId={me?.id} enterToSend={textMedia.enterToSend} conversations={(data?.conversations||[]).map(c=>({id:c.id,unread:c.unread||Number(navigationPreferences().unread.includes(c.id)),muted:prefs.muted.includes(c.id)}))} onSelectRoom={select} onMarkAllRead={readAction.available&&!data?.preview?()=>void readAction.run():undefined}/><CommandPalette onMessage={startDm} onSelectRoom={select} onSelectServer={chooseServer} onSettings={openSettings} onSearch={()=>setModal('search')} onCreate={!data?.preview&&(!currentServer||canEditCommunity(currentServer.id,'layout'))?()=>setModal('create'):undefined} onContacts={()=>openSettings('social')}/>
