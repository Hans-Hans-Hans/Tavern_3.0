...
function Workspace() {
    ...
    const loadBootstrap = useCallback(async () => { const b = await api('bootstrap'); setData(b); setPrefs({ ...defaultPrefs, ...b.preferences }); setProfileName(b.me.name); setWorkspaceName(b.workspace.name); if (!b.preview) {
        ...
        if(linkedServer){setSelectedServer(linkedServer.id);setGuild(b.conversations.find((c:Conv)=>linkedServer.roomIds.includes(c.id))?.id||'');pendingRoom.current='';return b as Bootstrap;}
        if (linked && b.conversations.some((c: Conv) => c.id === linked)) {
            setGuild(linked);
            pendingRoom.current = '';
            if(pendingEvent.current){const eventId=pendingEvent.current;pendingEvent.current='';void resolveMatrixMessage(linked,eventId).then(message=>{if(message.parent_id)return resolveMatrixMessage(linked,message.parent_id);return message;}).then(setThread).catch(e=>toast.error(e.message));}
        }
        else {
    ...
    useEffect(()=>{const follow=()=>{const link=parseConversationLink(location.hash);if(!link.roomId)return;pendingRoom.current=link.roomId;pendingEvent.current=link.eventId;void loadBootstrap();};window.addEventListener('hashchange',follow);return()=>window.removeEventListener('hashchange',follow);},[loadBootstrap]);
    const select = (id: string) => { setFocusMessage(null);setUnreadStart(null); if(currentServer&&!currentServer.roomIds.includes(id))setSelectedServer('all'); setGuild(id); setView('channel'); setTab('messages'); setThread(null); setOpenMobile(false); };
    ...
    function messageCard(m: Msg, inThread = false) {
        ...
        const gallery = (inThread ? [thread,...replies].filter(Boolean) as Msg[] : messages).flatMap(item=>item.attachments.map(attachment=>({...attachment,messageUrl:messageLink(item)})));
