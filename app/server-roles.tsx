...
type Props = { serverId: string; enabled: boolean; onChanged?: () => Promise<unknown> };
export function ServerRoles(props: Props) { return <ServerRoleEditor key={props.serverId} {...props}/>; }
function ServerRoleEditor({ serverId, enabled, onChanged }: Props) {
  const owner = useRoleEditorOwner(serverId), original = readRolePolicy(serverId);
  ...
  const [channel, setChannel] = useState(''), [targetType, setTargetType] = useState<'roles' | 'users'>('roles'), [target, setTarget] = useState('');
  ...
  const current = () => owner.current() && canManageServerRoles(serverId);
  ...
  async function save() {
    ...
    try { const saved = await saveRolePolicy(serverId, policy, previous); if (!current()) return; setPolicy(saved); setPrevious(saved); await onChanged?.(); if (current()) toast.success('Server roles saved'); }
  ...
  return <section className="channel-admin server-roles-editor">
    <h3>Server roles and permissions</h3>
    <p className="login-help">Give each role a recognizable look, choose its permissions, and assign it to members. Higher roles can manage only roles and members below them. Native Matrix permissions also apply.</p>
    <form className="dialog-form" onSubmit={event => { event.preventDefault(); void save(); }}>
      <fieldset disabled={busy}>
        ...
        {grants.has('manage_channels') && <details className="role-advanced"><summary>Channel overrides</summary>
          <p>Channels inherit category permissions first. Choose a role or member to override an individual channel.</p>
          <label>Channel<select value={channel} onChange={event => setChannel(event.target.value)}><option value="">Choose a channel</option>{serverChannelIds(serverId).map(id => <option key={id} value={id}>{owner.client?.getRoom(id)?.name || id}</option>)}</select></label>
        ...
        <div className="role-save-bar"><span role="status">{dirty ? 'Unsaved role changes' : 'No unsaved changes'}</span><button className="primary-button" disabled={!!previous && !dirty}>{busy ? 'Saving…' : previous ? 'Save roles and permissions' : 'Enable server roles'}</button><button type="button" className="secondary-button" onClick={reload}>Reload saved roles</button></div>
      </fieldset>
