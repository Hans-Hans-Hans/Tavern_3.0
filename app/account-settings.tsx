import { enableEmailHistory } from '@/lib/email-history';
...
export function AccountSettings() {
  ...
  return <div className="product-section"><h3>Account security</h3>{error && <p className="connect-error" role="alert">{error}</p>}
    {!security ? <p role="status">Loading account security…</p> : <>
      <form className="dialog-form" onSubmit={e => { e.preventDefault(); void run(() => requestApi('/account/password', { currentPassword: password, newPassword, confirmation, logoutOtherDevices: true, code, method, challengeId:emailChallenge }).then(async result=>{try{await enableEmailHistory(newPassword);}catch{toast.info('Password changed. Update email history recovery in Privacy settings from a device with your history keys.');}return result;}), 'Password changed. Other devices were signed out.'); }}>
