...
export function KeyboardShortcuts({ currentRoomId, conversations, onSelectRoom, onMarkAllRead, userId = '', enterToSend = true }: { currentRoomId: string; conversations: UnreadConversation[]; onSelectRoom: (roomId: string) => void; onMarkAllRead?: () => void; userId?: string; enterToSend?: boolean }) {
  ...
  useEffect(() => {
    ...
    const handle = (event: KeyboardEvent) => {
      ...
      if (action === 'mark-all-read') { if (onMarkAllRead) { event.preventDefault(); onMarkAllRead(); } return; }
  ...
  }, [binding, currentRoomId, conversations, onSelectRoom, onMarkAllRead, open]);
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogContent className='tavern-dialog keyboard-reference' onCloseAutoFocus={event => { event.preventDefault(); if (returnFocus.current?.isConnected) returnFocus.current.focus(); }}><DialogHeader><DialogTitle>Keyboard shortcuts</DialogTitle><DialogDescription>Use Ctrl on Windows and Linux, or Command on macOS. Unread navigation follows your conversation order and skips muted conversations.</DialogDescription></DialogHeader><table><thead><tr><th>Action</th><th>Shortcut</th></tr></thead><tbody>{[
    ['Switch conversation or find a person', 'Ctrl / ⌘ + K'], ['Command palette', 'Ctrl / ⌘ + Shift + P'], ['Search messages', 'Ctrl / ⌘ + F'], ['This reference', 'Ctrl / ⌘ + /'], ['Previous unread conversation', binding === 'disabled' ? 'Disabled' : navigationKeys + '↑'], ['Next unread conversation', binding === 'disabled' ? 'Disabled' : navigationKeys + '↓'], ['Mark all joined conversations read through loaded events', 'Alt + Shift + R'], ['Close menu or dialog; cancel an edit', 'Esc'], ['Choose a menu item', '↑ / ↓, then Enter'], ['Send message', enterToSend ? 'Enter' : 'Ctrl / ⌘ + Enter'], ['Insert a new line', enterToSend ? 'Shift + Enter' : 'Enter'], ['Edit your last message with an empty composer', '↑'],
