...
function UserDetails({ userId, operator, onChanged }: { userId: string; operator: AccountSession; onChanged: () => void }) {
  ...
  const { user, security, storage } = value, protectedAccount = value.serviceAccount || userId === operator.userId;
  return <><p><strong>{user.displayname || user.name}</strong> · {status(user, security)} · {user.admin ? 'Instance administrator' : 'User'}</p>
    ...
    <Tabs defaultValue='profile' className='settings-tabs'><TabsList aria-label='User administration sections'><TabsTrigger value='profile'>Profile</TabsTrigger><TabsTrigger value='security'>Security</TabsTrigger><TabsTrigger value='rooms'>Rooms</TabsTrigger><TabsTrigger value='sessions'>Sessions</TabsTrigger><TabsTrigger value='storage'>Storage</TabsTrigger><TabsTrigger value='audit'>Audit</TabsTrigger></TabsList>
      ...
      <TabsContent value='security' className='product-section'><p>Two-step verification: {mfaStatus(security)}<br/>Password change: {security.managed ? security.passwordChangeRequired ? 'Required before returning to Tavern' : 'Not required' : 'Not managed by Tavern'}<br/>Tavern access: {security.accessBlocked ? 'Blocked' : 'Allowed, subject to account status'}</p>{protectedAccount && <p>{value.serviceAccount ? 'Service account access changes require deployment maintenance.' : 'Use Account security to manage your own password, verification, and sessions.'}</p>}<div className='product-actions'>
        <button className='secondary-button' disabled={busy || protectedAccount || user.deactivated} onClick={() => setAction('require_password_change')}>Require password change…</button>
        <button className='secondary-button' disabled={busy || protectedAccount || user.deactivated || !security.managed || !security.mfaMethods?.length} onClick={() => setAction('reset_mfa')}>Reset two-step verification…</button>
        <button className='secondary-button' disabled={busy || protectedAccount || user.deactivated} onClick={() => setAction('revoke_sessions')}>Revoke all sessions…</button>
        {user.deactivated || user.suspended || security.accessBlocked ? <button className='secondary-button' disabled={busy || protectedAccount} onClick={() => setAction('enable')}>Enable account…</button> : <button className='secondary-button danger-text' disabled={busy || protectedAccount} onClick={() => setAction('suspend')}>Suspend account…</button>}
...
const actionLabels: Record<Action, string> = { revoke_sessions: 'Revoke all sessions', reset_mfa: 'Reset two-step verification', require_password_change: 'Require password change', suspend: 'Suspend account', enable: 'Enable account', deactivate: 'Deactivate account', set_quota: 'Set user quota' };
...
function UserAction({ action, value, onDone }: { action: Action; value: UserDetail; onDone: () => void }) {
  ...
  return <form className='dialog-form' onSubmit={async event => {
    ...
    try {
      ...
      await requestApi('/admin/users/' + encodeURIComponent(value.user.name) + '/actions', { action, confirmation, password, ...(needsMfa ? { method, code, challengeId } : {}), ...(action === 'set_quota' ? { quotaBytes } : {}), ...(needsPassword ? { newPassword } : {}) });
      setPassword(''); setCode(''); setNewPassword(''); setRepeatPassword(''); toast.success(actionLabels[action] + ' completed'); onDone();
    } catch (e: any) { setError(e.message); } finally { setBusy(false); }
