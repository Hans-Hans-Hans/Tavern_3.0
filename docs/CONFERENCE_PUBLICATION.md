...
Fifteen focused native/API tests cover explicit migration, monotonic markers, exact revisions, current authority, all-parent inheritance, category/channel precedence, protected unlink/redaction, signed token attenuation, old-token rejection, active participant updates, observed permission ceilings, restoration, and incompatible/disabled runtime behavior. Frontend semantic and mounted browser tests cover native revision preservation, malformed markers, concurrent assignments, account changes, migration with unsaved drafts, and the actual role editor.
