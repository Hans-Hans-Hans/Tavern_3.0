...
Validation: 54 focused Python tests passed in the independent review, and all four original HTTP reproductions now fail closed: delayed privacy, ambiguous native success, delayed invitation authorization, and delayed registration. Account/admin/auth browser regressions also passed. The final isolated CI probe, `scripts/smoke-deactivation.mjs`, exercises Bob's real settings dialog, native state, journal completion, and rejection of his former session cookie after the other live checks. Its syntax and scope guards passed locally; that new native Synapse acceptance run is still pending.
