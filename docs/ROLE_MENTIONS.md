...
The isolated live CI module, `scripts/smoke-role-mentions.mjs`, creates a fresh marked Space and encrypted child using the existing Alice and Bob owning browsers. It exercises the actual composer picker, native role hierarchy, stable selection after a rename, disabling a selected role and literal code examples. Native event reads verify ciphertext; each owning browser's shipped history exporter independently decrypts only this room and checks the exact `m.mentions` content. Its scope guards run locally, but the new live acceptance remains pending until its CI run passes. It does not test operating-system notifications, and it does not read or change preexisting room policies. Any temporary change to Bob's own global invitation preference is restored; the isolated stack teardown owns the new rooms.

The probe accepts native version 12 hash room IDs and checks their inspected version, immutable local creator and unique run marker. It does not infer locality from a domain suffix: [version 12 removes that suffix and gives creators inherent power](https://spec.matrix.org/v1.16/rooms/v12/#client-considerations). Legacy fixture IDs must still use the exact isolated server domain.
