...
Use a supported pinned Synapse release. The call configuration was written against Synapse **1.160.0**, LiveKit **1.13.6**, RTC authorization **0.6.0**, and Element Call embedded **0.25.0**. Update the `SYNAPSE_IMAGE` stack setting to `matrixdotorg/synapse:v1.160.0` after following the Synapse upgrade notes and backing up the database/configuration. These versions have not been booted together in this build environment.
...
Use the conference button for encrypted group calls. Its bundled call UI provides microphone/camera controls, screen sharing, participant layout, and device selection. Leaving the channel closes this version's conference panel; it does not yet provide a persistent cross-channel meeting dock. Direct voice/video buttons are available in DMs with exactly two joined members. Direct calls require self-hosted TURN and use relay-only connections. The participant-verification control is separate from media encryption.

In **Settings → Privacy**, a new account can create a signing identity and encrypted backup after saving its recovery key. On a later device, enter the same recovery key to unlock identity and automatic recovery, or compare emoji with another device. Full-history key restore is optional. If setup was interrupted, select **Finish interrupted setup** using the saved key. Tavern preserves existing backup versions instead of using the SDK's destructive backup reset operation.
