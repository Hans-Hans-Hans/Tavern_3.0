...
function fixture({identity=false,storage=false,backup=false}={}) {
  ...
  const api=loadTs('../lib/security.ts',{'./backup':{discoverBackup:async()=>backup?{version:'1'}:null,createOrResumeBackup:async()=>{backup=true;}},'matrix-js-sdk/lib/crypto-api':{CryptoEvent:{},VerificationPhase:{Cancelled:5,Done:6},VerificationRequestEvent:{Change:'change'},VerifierEvent:{ShowSas:'sas',Cancel:'cancel'},decodeRecoveryKey:s=>{if(s!=='valid')throw new Error('Invalid key');return new Uint8Array(32).fill(7);}}});
  const crypto={
