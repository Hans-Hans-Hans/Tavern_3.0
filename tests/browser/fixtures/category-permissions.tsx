...
export function mountFixture() {
  ...
  w.fixtureClient = { getUserId: () => '@owner:local', getRoom: () => room, getStateEvent: async (_room: string, type: string) => structuredClone(type === 'io.tavern.roles' ? w.policy : w.layout), sendStateEvent: async (_room: string, _type: string, value: unknown) => { w.policy = structuredClone(value); w.saves = (w.saves || 0) + 1; } };
