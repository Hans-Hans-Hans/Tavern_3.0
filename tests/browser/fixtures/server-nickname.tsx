...
export function mountFixture() {
  ...
  const wrap = (raw: any) => raw && ({ getContent: () => raw.content, getId: () => raw.event_id, getSender: () => raw.sender });
  const room = { name: 'Test server', isSpaceRoom: () => true, getMyMembership: () => 'join', getMember: (user: string) => ({ name: w.state.find((e: any) => e.type === 'm.room.member' && e.state_key === user)?.content.displayname }),
    currentState: { getStateEvents: (type: string, key: string) => wrap(w.state.find((e: any) => e.type === type && e.state_key === key)) } };
