...
export function mountFixture() {
  ...
  const f: any = { account: 0, actor: params.get('actor') === 'mod' ? mod : owner, device: 'ROLE_DEVICE', listeners: new Set(), writes: [], changed: 0, membership: 'join', holdRead: false, holdWrite: false, revision: 0 };
  ...
  const room = { roomId: server, name: 'Test server', isSpaceRoom: () => true, getMyMembership: () => f.membership, getJoinedMembers: () => f.members,
    getMember: (user: string) => f.members.find((item: any) => item.userId === user), currentState: {
      maySendStateEvent: () => true, getStateEvents: (kind: string) => {
        if (kind === 'm.space.child') return [{ getStateKey: () => channel, getContent: () => ({ via: ['test'] }) }];
  ...
  f.client = { getUserId: () => f.actor, getDeviceId: () => f.device,
    getRoom: (id: string) => id === server ? room : { name: 'Voice lounge' },
    ...
    roomState: async () => { const value = [{ type: 'io.tavern.roles', state_key: '', event_id: '$roles-' + f.revision, content: structuredClone(f.policy) }]; await waitRead(); return value; },
  ...
  createRoot(document.getElementById('root')!).render(<main style={{ padding: 12, maxWidth: 1040, margin: '0 auto' }}><Toaster/>{mode === 'member'
    ...
    : <ServerRoles serverId={server} enabled onChanged={async () => { f.changed++; }}/>}</main>);
}
