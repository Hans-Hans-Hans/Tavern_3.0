...
async function fixture(page: Page, text: string, search = false, preview = false) {
  ...
  await page.route(url => url.pathname === '/lib/matrix.ts', route => route.fulfill({ contentType: 'application/javascript', body: 'const room={roomId:"!guild:test",name:"Guild",getMyMembership:()=>"join",isSpaceRoom:()=>false,getJoinedMembers:()=>[],getMember:()=>null,findEventById:()=>null,currentState:{getStateEvents:()=>({getContent:()=>({emoji:[{name:"cheers",uri:"mxc://test/emoji",creator:"@owner:test"}]})})}};const client={getUserId:()=>"@alice:test",getDeviceId:()=>"MARKDOWN",getIgnoredUsers:()=>[],getRoom:()=>room,getRooms:()=>[room]};export const getMatrixClient=()=>client;export const onMatrixUpdate=()=>()=>{};export const resolveMatrixMessage=async(roomId,id)=>({id,conversation_id:roomId});export const matrixFileBlob=async()=>{throw new Error("No attachment fixture");};export const downloadMatrixFile=async()=>{throw new Error("No attachment fixture");};' }));
  ...
  await page.route(url => url.pathname === '/lib/search-index.ts', route => route.fulfill({ contentType: 'application/javascript', body: 'export const searchMessages=async()=>({hits:[{id:"$result",roomId:"!guild:test",roomName:"Guild",authorId:"@alice:test",authorName:"Alice",timestamp:1,body:window.initialMarkdown,has:[]}],nextCursor:null,indexedCount:1});export const searchIndexStatus=async()=>({indexedCount:1});export const indexRoomHistory=async()=>{};export const clearSearchIndex=async()=>{};' }));
