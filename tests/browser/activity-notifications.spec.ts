...
async function fixture(page: Page) {
  ...
  await page.route(url => url.pathname === '/lib/api.ts', route => route.fulfill({ contentType: 'text/javascript', body: `const owner={};export const accountArtworkOwner=()=>owner;export const isManagedAccount=()=>true;export const requestApi=async()=>{window.reads++;if(window.waitNext){window.waitNext=false;await new Promise(resolve=>window.finishRead=resolve);}return structuredClone(window.social);};` }));
