...
// outer modal are fixture boundaries.
...
visit(source);
if (!header) throw new Error('The actual workspace header must be present.');
...
export function WorkspaceMenu(){const[selected,setSelected]=useState('all'),[modal,setModal]=useState('');const data={workspace:{name:'Tavern'},servers:[f.server]},currentServer=selected===f.server.id?f.server:null,terms={server:'server'},chooseServer=setSelected,canInviteToRoom=()=>false,openSettings=()=>{};return <><style>{'[data-slot="dropdown-menu-content"][data-state="closed"]{animation-duration:1000ms!important}'}</style>${header}{modal&&<div role='dialog'>{modal}</div>}</>}
...
test('an immediate toggle during the real Radix exit leaves settings in a closing menu', async ({ page }) => {
  await fixture(page);
  await page.locator('.workspace-select').click();
  await page.getByRole('menuitem', { name: 'CI system notices test server', exact: true }).click();
  await expect(page.locator('.workspace-select')).toHaveAttribute('aria-expanded', 'false');
  ...
  await expect(page.getByRole('menuitem', { name: 'Server settings & categories', exact: true })).toHaveCount(0);
...
test('the menu helper cannot bypass native permission when the actor is an ordinary joined member', async ({ page }) => {
  ...
  await expect(openSystemMessageSettings(page, 'CI system notices test server')).rejects.toThrow(/Server settings & categories/);
