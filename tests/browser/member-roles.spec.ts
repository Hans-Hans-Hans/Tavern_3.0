...
test('member role checkboxes enforce grants and preserve concurrent assignments and permissions', async ({ page }) => {
  ...
  await page.route('**/member-roles-test', route => route.fulfill({ contentType: 'text/html', body: `<!doctype html><html><body><div id="root"></div><script type="module">import RefreshRuntime from '/@react-refresh';RefreshRuntime.injectIntoGlobalHook(window);window.$RefreshReg$=()=>{};window.$RefreshSig$=()=>type=>type;window.__vite_plugin_react_preamble_installed__=true;(await import('/tests/browser/fixtures/member-roles.tsx')).mountFixture();</script></body></html>` }));
  await page.goto('/member-roles-test'); await expect(page.getByRole('checkbox', { name: 'Privileged', exact: true })).toBeDisabled(); await expect(page.getByRole('checkbox', { name: 'Moderator', exact: true })).toBeDisabled(); await page.getByRole('checkbox', { name: 'Tag', exact: true }).check();
  await page.evaluate(() => { const w = window as any; w.policy.members['@target:local'] = ['other']; w.policy.members['@unrelated:local'] = ['tag']; w.policy.categoryOverrides.cat = { roles: { everyone: { send_messages: -1 } }, users: {} }; });
