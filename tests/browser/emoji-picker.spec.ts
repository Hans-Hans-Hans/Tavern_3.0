...
test('unified emoji picker supports tones, durable favorites/frequency, keyboard search and server emoji', async ({ page }) => {
  await page.route(url => url.pathname === '/lib/matrix.ts', route => route.fulfill({ contentType: 'text/javascript', body: 'export const mutateMatrixAccountData=()=>{throw Error("Unexpected account-data write");};export const getMatrixClient=()=>window.fixtureClient;export const onMatrixUpdate=()=>()=>{};export const matrixApi=async()=>({});' }));
