...
function fixture() {
  ...
  const api = loadTs('../lib/channel-policy.ts', { './member-state': loadTs('../lib/member-state.ts', {}), './matrix': { getMatrixClient: () => client }, './roles': { readRolePolicy: () => null, effectiveRolePermissions: () => new Set(), memberRoleRank: () => 0 } });
...
test('timeout writes are bound to the selected member and expiry', async () => { const f = fixture(); await f.api.timeoutMember('!room:test', '@member:test', 60, 'Cooling off'); assert.equal(f.writes[0][1], 'io.tavern.timeout'); assert.equal(f.writes[0][3], '_member:test'); assert.ok(f.writes[0][2].until > Date.now()); });

test('timeout migration retains the observed legacy revision and canonical clear takes precedence in composer state', async () => {
  const f = fixture(), room = f.client.getRoom(), legacy = { getContent: () => ({ until: Date.now() + 100000 }), getId: () => '$legacy' };
  let current;
  room.currentState.getStateEvents = (type, key) => key === undefined ? [] : type !== 'io.tavern.timeout' ? null : key === '_member:test' ? current : key === '@member:test' ? legacy : null;
  assert.ok(f.api.memberTimeout('!room:test', '@member:test').until > Date.now());
  await f.api.timeoutMember('!room:test', '@member:test', 0);
  assert.equal(f.writes[0][2]['io.tavern.previous_event'], '$legacy');
  current = { getId: () => '$clear', getContent: () => f.writes[0][2] };
  assert.equal(f.api.memberTimeout('!room:test', '@member:test').until, 0);
});
...
test('composer inherits temporary bans only through reciprocal canonical server parents', () => {
  ...
  const api = loadTs('../lib/channel-policy.ts', { './member-state': loadTs('../lib/member-state.ts', {}), './matrix': { getMatrixClient: () => client }, './roles': { readRolePolicy: id => id === '!server:test' ? {} : null, effectiveRolePermissions: () => new Set(), memberRoleRank: () => 0 } });
